...
aws s3api create-bucket --bucket teratree-media --create-bucket-configuration LocationConstraint=$AWS_DEFAULT_REGION
aws s3api put-bucket-policy --bucket teratree-media --policy '{
    "Version": "2012-10-17",
    "Statement": [
        {
            "Sid": "AddPerm",
            ...
            "Principal": "*",
            "Action": "s3:GetObject",
            "Resource": "arn:aws:s3:::teratree-media/*"
        }
    ]
}'
aws s3api put-bucket-cors --bucket teratree-media --cors-configuration='{
  "CORSRules": [
    {
      "AllowedOrigins": ["*"],
      "AllowedMethods": ["GET"],
...
aws iam create-user --user-name teratree-media-user
export BUCKET_CREDS=`aws iam create-access-key --user-name teratree-media-user --query "AccessKey.{AccessKeyId:AccessKeyId,SecretAccessKey:SecretAccessKey}" --output=text`
...
aws iam put-user-policy --user-name teratree-media-user --policy-name 'teratree-media-access' --policy-document '{
    "Version": "2012-10-17",
    "Statement": [
        {
            "Sid": "VisualEditor0",
