...
EMAIL_BACKEND = 'teratree.emailbackend.LoggingBackend'
ADMINS = []
...
```
aws iam create-user --user-name teratree-smtp
aws iam put-user-policy --user-name teratree-smtp --policy-name 'send-email' --policy-document '{ "Statement": [{"Effect":"Allow", "Action":"ses:SendRawEmail", "Resource":"*" }]}'
export CREDS=`aws iam create-access-key --user-name teratree-smtp --query "AccessKey.{AccessKeyId:AccessKeyId,SecretAccessKey:SecretAccessKey}" --output=text`
